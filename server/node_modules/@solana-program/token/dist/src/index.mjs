...
function tokenProgram2() {
  return (client) => {
    return pipe(client, tokenProgram(), (c) => ({
      ...c,
      token: {
        ...c.token,
        instructions: {
          ...c.token.instructions,
          createMint: (input) => addSelfPlanAndSendFunctions(
            client,
            getCreateMintInstructionPlan({ ...input, payer: input.payer ?? client.payer })
          ),
          mintToATA: (input) => addSelfPlanAndSendFunctions(
            client,
            getMintToATAInstructionPlan({ ...input, payer: input.payer ?? client.payer })
          ),
          transferToATA: (input) => addSelfPlanAndSendFunctions(
            client,
