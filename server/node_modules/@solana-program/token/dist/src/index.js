...
function tokenProgram2() {
  return (client) => {
    return kit.pipe(client, tokenProgram(), (c) => ({
      ...c,
      token: {
        ...c.token,
        instructions: {
          ...c.token.instructions,
          createMint: (input) => programClientCore.addSelfPlanAndSendFunctions(
            client,
            getCreateMintInstructionPlan({ ...input, payer: input.payer ?? client.payer })
          ),
          mintToATA: (input) => programClientCore.addSelfPlanAndSendFunctions(
            client,
            getMintToATAInstructionPlan({ ...input, payer: input.payer ?? client.payer })
          ),
          transferToATA: (input) => programClientCore.addSelfPlanAndSendFunctions(
            client,
