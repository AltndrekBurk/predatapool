    ...
    return Method.toServer(Methods.session, {
        ...
        async verify({ credential, envelope, request }) {
            ...
            switch (payload.action) {
                case 'open':
                    sessionReceipt = await handleOpen(store, client, challenge, payload, methodDetails, resolvedFeePayer, feePayerPolicy, waitForConfirmation);
                    lastOnChainVerified.set(sessionReceipt.channelId, Date.now());
                    break;
                case 'topUp':
                    sessionReceipt = await handleTopUp(store, client, challenge, payload, methodDetails, resolvedFeePayer, feePayerPolicy);
                    lastOnChainVerified.set(sessionReceipt.channelId, Date.now());
                    break;
                case 'voucher':
 ...
 */
async function handleOpen(store, client, challenge, payload, methodDetails, feePayer, feePayerPolicy, waitForConfirmation) {
 ...
 */
async function handleTopUp(store, client, challenge, payload, methodDetails, feePayer, feePayerPolicy) {
    const channelId = ChannelStore.normalizeChannelId(payload.channelId);
    const channel = await store.getChannel(channelId);
    if (!channel) {
        throw new ChannelNotFoundError({ reason: 'channel not found' });
...
async function handleVoucher(store, client, minVoucherDelta, challenge, payload, methodDetails, channelStateTtl, lastOnChainVerified) {
    ...
    const onChain = await (async () => {
        if (isStale) {
            const onChainChannel = await getOnChainChannel(client, methodDetails.escrowContract, channelId);
